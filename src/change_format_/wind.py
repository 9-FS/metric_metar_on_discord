...
def change_format_wind(info: str, station: dict, RWY_DB: pandas.DataFrame) -> str|None:
    ...
    if re_match!=None:
        ...
        if wind_unit=="KT": #if kt: convert
            wind_speed     *=KFS.convert_to_SI.speed["kt"]
            wind_speed_gust*=KFS.convert_to_SI.speed["kt"]


        info_new=f"VRB{KFS.fstr.notation_abs(wind_speed, 0, round_static=True, width=2)}"
        if wind_speed!=wind_speed_gust:
            info_new+=f"G{KFS.fstr.notation_abs(wind_speed_gust, 0, round_static=True, width=2)}"
        info_new+="m/s"
    ...
    if re_match!=None:
        ...
        info_new=f"{KFS.fstr.notation_abs(wind_direction, 0, round_static=True, width=3)}°{KFS.fstr.notation_abs(wind_speed, 0, round_static=True, width=2)}"
        if wind_speed!=wind_speed_gust:
            info_new+=f"G{KFS.fstr.notation_abs(wind_speed_gust, 0, round_static=True, width=2)}"
        info_new+="m/s"
    ...
    if re_match!=None:
        ...
        return f" {KFS.fstr.notation_abs(wind_direction_1, 0, round_static=True, width=3)}°V{KFS.fstr.notation_abs(wind_direction_2, 0, round_static=True, width=3)}°"